...
const App: React.FC = () => {
  ...
  const getBreathSize = () => {
    // 進一步調整圓圈大小與發光強度，確保在移動端不超出視區
    if (breathPhase === 'inhale') return 'w-48 h-48 bg-cyan-300 shadow-[0_0_80px_rgba(34,211,238,0.8)] sm:w-56 sm:h-56';
    if (breathPhase === 'hold') return 'w-48 h-48 bg-white shadow-[0_0_100px_rgba(255,255,255,0.9)] scale-105 sm:w-56 sm:h-56';
    if (breathPhase === 'exhale') return 'w-10 h-10 bg-cyan-400/40 shadow-[0_0_20px_rgba(34,211,238,0.3)] sm:w-12 sm:h-12';
    return 'w-10 h-10 bg-cyan-400/20 sm:w-12 sm:h-12';
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-2 bg-zinc-950 scanlines overflow-hidden font-sans select-none" onContextMenu={(e) => e.preventDefault()}>
      ...
      <div className="w-full max-w-5xl h-full bg-zinc-900 md:border-[8px] border-zinc-800 md:rounded-[48px] shadow-2xl relative flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex justify-between items-center bg-zinc-900/95 border-b-2 border-zinc-800 p-3 relative z-50 h-16 shrink-0">
        ...
        <main className="flex-1 p-4 relative overflow-hidden flex flex-col min-h-0">
          {state.status === 'splash' && (
          ...
          {state.status === 'routing' && (
            <div className="flex-1 flex flex-col items-center justify-center animate-in zoom-in h-full overflow-y-auto pt-4 pb-8">
          ...
          {state.status === 'playing' && (
            <div className="flex-1 flex flex-col items-center justify-center gap-6 sm:gap-10">
              {state.currentCard && (
                <div className="w-full flex flex-col items-center gap-4 sm:gap-8 animate-in zoom-in">
                  <div onClick={startScanningCard} className={`relative w-[240px] h-[340px] sm:w-[280px] sm:h-[400px] cursor-pointer transition-all duration-700 preserve-3d ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-br ${state.currentCard.type === '開心' ? 'from-emerald-950/90 to-zinc-950' : 'from-rose-950/90 to-zinc-950'} border-4 ${state.currentCard.type === '開心' ? 'border-emerald-500/50 shadow-emerald-500/20' : 'border-rose-500/50 shadow-rose-500/20'} rounded-[32px] flex flex-col p-6 [transform:rotateY(180deg)] backface-hidden shadow-[0_0_40px_rgba(0,0,0,0.5)]`}>
                       ...
                       <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center gap-4 sm:gap-6">
                          <div className={`p-6 sm:p-8 bg-zinc-900/80 rounded-full border-2 ${state.currentCard.type === '開心' ? 'border-emerald-500/30' : 'border-rose-500/30'} backdrop-blur-sm shadow-inner`}>
                             {React.createElement(ICON_MAP[state.currentCard.icon] || Rocket, { className: `w-12 h-12 sm:w-14 sm:h-14 ${state.currentCard.type === '開心' ? 'text-emerald-400' : 'text-rose-400'}` })}
                          </div>
                          <h3 className="text-lg sm:text-xl font-black text-white leading-relaxed drop-shadow-md">{state.currentCard.cn}</h3>
                       </div>
                  ...
                  {isFlipped && !isScanning && (
                    <div className="flex gap-4 sm:gap-6 w-full max-w-xs sm:max-w-md px-4 animate-in slide-in-from-bottom">
                      <button onClick={() => handleDecision('開心')} className="flex-1 py-4 sm:py-5 bg-emerald-600/10 border-2 border-emerald-500 text-emerald-400 rounded-2xl font-black text-lg sm:text-xl hover:bg-emerald-600/20 active:translate-y-1 transition-all shadow-lg">開心</button>
                      <button onClick={() => handleDecision('不開心')} className="flex-1 py-4 sm:py-5 bg-rose-600/10 border-2 border-rose-500 text-rose-400 rounded-2xl font-black text-lg sm:text-xl hover:bg-rose-600/20 active:translate-y-1 transition-all shadow-lg">不開心</button>
                    </div>
                  )}
                </div>
              )}
          ...
          {state.status === 'sop' && (
            <div className="flex-1 flex flex-col bg-zinc-950 rounded-[40px] border-2 border-rose-500/30 overflow-hidden shadow-2xl relative max-h-full">
               <div className="bg-rose-700 text-white p-3 flex justify-between items-center z-20 shrink-0">
                 <div className="flex items-center gap-2 font-header text-[10px] sm:text-sm tracking-widest animate-pulse uppercase"><ShieldAlert size={16} className="sm:w-[18px]" /> Stabilization Protocol</div>
                 <div className="bg-black/30 px-3 py-1 rounded-full text-[8px] sm:text-[10px] font-black uppercase">Phase 0{state.sopStep}</div>
               </div>
                 ...
                 {isDecompressing && (
                   <div className="absolute inset-0 z-50 bg-cyan-900/90 backdrop-blur-xl flex flex-col items-center justify-center text-white">
                      <div className="relative">
                        <Wind className="w-20 h-20 sm:w-24 sm:h-24 text-cyan-300 animate-spin-slow opacity-80" />
                        <div className="absolute inset-0 animate-ping bg-cyan-400 rounded-full opacity-20 scale-150" />
                      </div>
                      <h3 className="text-2xl sm:text-3xl font-header mt-8 tracking-widest uppercase italic">核心散熱中...</h3>
                      <div className="mt-4 text-[10px] font-mono-tech text-cyan-200 uppercase tracking-widest">Releasing Emotional Pressure</div>
                   </div>
                 )}

                 <div className="flex-1 w-full flex flex-col items-center justify-center min-h-0 overflow-y-auto pt-2 pb-4">
                   {state.sopStep === 1 && (
                     <div className="text-center animate-in zoom-in w-full max-w-xs shrink-0 py-2">
                       <h3 className="text-xl sm:text-2xl font-black text-white mb-1 uppercase tracking-widest">核心加壓</h3>
                       <p className="text-zinc-500 text-[10px] mb-6 italic">長按進行核心同步感應</p>
                       
                       <div className="relative w-48 h-48 sm:w-56 sm:h-56 mx-auto group shrink-0">
                          {isPressing && (
                            <div className="absolute inset-0 animate-ping bg-rose-500/20 rounded-full scale-125" />
                          )}
                          ...
                          <button 
                            ...
                            style={{ 
                              transform: isPressing ? `translate(${Math.random() * (pressure/10) - pressure/20}px, ${Math.random() * (pressure/10) - pressure/20}px) scale(${1 + pressure/1000})` : 'scale(1)' 
                            }}
                            className={`absolute inset-4 rounded-full border-4 transition-all flex flex-col items-center justify-center ${pressure >= 100 ? 'bg-emerald-600 border-emerald-400 scale-110 shadow-[0_0_40px_emerald]' : 'bg-rose-600 border-rose-400 shadow-lg active:shadow-rose-500/50'}`}
                          >
                            <span className="text-white font-black text-lg sm:text-xl uppercase tracking-tighter">{pressure >= 100 ? 'STABLE' : isPressing ? '壓力釋放' : 'HOLD'}</span>
                   ...
                   {state.sopStep === 2 && (
                     <div className="text-center w-full max-w-sm animate-in slide-in-from-right flex flex-col items-center shrink-0 py-2">
                       <h3 className="text-xl sm:text-2xl font-black text-white mb-1 tracking-widest uppercase">熱能冷卻 / Cool Down</h3>
                       <p className="text-zinc-500 text-[10px] mb-4 sm:mb-6 italic">呼吸 {state.missionConfig.breathCycles} 次，進行核心降溫</p>
                       
                       <div className="relative w-56 h-56 sm:w-64 sm:h-64 border-4 border-cyan-500/10 rounded-full flex items-center justify-center mb-4 sm:mb-6 shrink-0">
                          <div 
                            className={`rounded-full transition-all ease-in-out z-10 ${getBreathSize()}`} 
                       ...
                       <div className="flex justify-center gap-2 mb-4 sm:mb-6 shrink-0">
                         {Array.from({ length: state.missionConfig.breathCycles }).map((_, i) => (
                           <div key={i} className={`w-8 sm:w-10 h-1.5 rounded-full transition-all duration-500 ${breathCount > i ? 'bg-cyan-400 shadow-[0_0_15px_cyan]' : 'bg-zinc-800'}`} />
                         ))}
                       </div>
                       
                       <button onClick={() => { playSfx('click'); setState(prev => ({ ...prev, sopStep: 3 })); }} disabled={breathCount < state.missionConfig.breathCycles} className="w-full py-3 sm:py-4 bg-cyan-600 disabled:opacity-20 text-white rounded-2xl font-black text-lg border-b-6 border-cyan-950 transition-all shadow-lg active:scale-95 shrink-0">確認穩定</button>
                     </div>
                   )}

                   {state.sopStep === 3 && (
                     <div className="w-full h-full flex flex-col animate-in slide-in-from-right py-2">
                        {state.currentCard && (
                          <div className="bg-zinc-900/80 border border-zinc-800 p-2 sm:p-3 rounded-2xl mb-3 flex items-center gap-3 animate-in fade-in duration-500 shrink-0">
                            <div className="p-1.5 sm:p-2 bg-rose-500/20 rounded-lg border border-rose-500/30">
                              {React.createElement(ICON_MAP[state.currentCard.icon] || HelpCircle, { className: 'text-rose-400 w-4 h-4 sm:w-5 sm:h-5' })}
                            </div>
                            <div className="flex-1 overflow-hidden">
                               <span className="text-[9px] text-zinc-500 font-bold block mb-0.5 uppercase tracking-tighter">偵測情境</span>
                               <span className="text-[11px] sm:text-xs text-white font-black block truncate">{state.currentCard.cn}</span>
                            </div>
                          </div>
                        )}

                        <div className="flex-1 bg-black/50 rounded-[24px] sm:rounded-[32px] p-4 sm:p-5 border border-zinc-800 overflow-y-auto space-y-4 sm:space-y-6 scrollbar-thin shadow-inner">
                           <div>
                              <label className="text-[10px] text-zinc-500 font-black block mb-2 sm:mb-3 pl-3 border-l-4 border-amber-500 uppercase tracking-widest">目前核心情緒 / Mood</label>
                              <div className="grid grid-cols-3 gap-2">
                                {EMOTIONS.map(e => (
                                  <button key={e.id} onClick={() => { playSfx('click'); setSopSelection(prev => ({ ...prev, emotion: e.id })); }} 
                                    className={`py-2.5 sm:py-3 rounded-xl border-2 flex flex-col items-center gap-1.5 transition-all active:scale-95 ${sopSelection.emotion === e.id ? 'bg-amber-500/20 border-amber-500 text-amber-300 scale-105 animate-bounce-short shadow-glow-amber' : 'bg-zinc-900/50 border-zinc-800 text-zinc-600'}`}>
                                    {React.createElement(ICON_MAP[e.icon] || HelpCircle, { size: 18, className: "sm:w-5 sm:h-5" })}
                                    <span className="text-[9px] sm:text-[10px] font-bold">{e.cn}</span>
                                  </button>
                                ))}
                              </div>
                           </div>
                           <div>
                              <label className="text-[10px] text-zinc-500 font-black block mb-2 sm:mb-3 pl-3 border-l-4 border-cyan-500 uppercase tracking-widest">核心行動方針 / Action</label>
                              <div className="grid grid-cols-3 gap-2">
                                {NEEDS.map(n => (
                                  <button key={n.id} onClick={() => { playSfx('click'); setSopSelection(prev => ({ ...prev, need: n.id })); }} 
                                    className={`py-2.5 sm:py-3 rounded-xl border-2 flex flex-col items-center gap-1.5 transition-all active:scale-95 ${sopSelection.need === n.id ? 'bg-cyan-500/20 border-cyan-500 text-cyan-300 scale-105 animate-bounce-short shadow-glow-cyan' : 'bg-zinc-900/50 border-zinc-800 text-zinc-600'}`}>
                                    {React.createElement(ICON_MAP[n.icon] || LifeBuoy, { size: 18, className: "sm:w-5 sm:h-5" })}
                                    <span className="text-[9px] sm:text-[10px] font-bold">{n.cn}</span>
                                  </button>
                                ))}
                              </div>
                           </div>
                        </div>
                        <button disabled={!sopSelection.emotion || !sopSelection.need} onClick={handleSopCompletion} className="mt-3 sm:mt-4 w-full py-3.5 sm:py-4 bg-emerald-600 disabled:opacity-20 text-white rounded-2xl font-black text-lg border-b-6 border-emerald-950 shadow-xl active:translate-y-1 transition-all shrink-0">執行修復協議</button>
                     </div>
                   )}
                 </div>
               </div>
          ...
          {state.status === 'result' && (
            <div className="flex-1 flex flex-col items-center justify-center text-center px-4 animate-in zoom-in overflow-y-auto py-6">
              <div className="w-full max-w-md bg-zinc-900 border-4 border-emerald-500/30 rounded-[48px] p-8 sm:p-10 shadow-2xl relative overflow-hidden shrink-0">
                ...
                <h2 className="text-2xl sm:text-3xl font-header text-white mb-2 uppercase tracking-widest">任務完成 / Success</h2>
                <p className="text-zinc-500 text-[10px] mb-8 sm:mb-10 font-mono-tech uppercase">Core Synchronization Complete</p>
                <div className="grid grid-cols-2 gap-4 mb-8 sm:mb-10">
                  <div className="bg-black/40 p-5 sm:p-6 rounded-3xl border border-zinc-800"><span className="block text-[8px] text-zinc-500 uppercase font-black mb-1">Score</span><span className="text-2xl sm:text-3xl text-emerald-400 font-header">{state.score}</span></div>
                  <div className="bg-black/40 p-5 sm:p-6 rounded-3xl border border-zinc-800"><span className="block text-[8px] text-zinc-500 uppercase font-black mb-1">XP Gain</span><span className="text-2xl sm:text-3xl text-amber-400 font-header">+{state.xp}</span></div>
                </div>
                <div className="flex flex-col gap-4">
                  <button onClick={startNewMission} className="w-full py-4 bg-emerald-600 text-white rounded-3xl font-black text-xl border-b-8 border-emerald-950 shadow-lg">再次同步</button>
                  <button onClick={() => setState(prev => ({ ...prev, status: 'splash' }))} className="w-full py-3 bg-zinc-800 text-zinc-500 rounded-3xl font-black text-sm hover:text-white border-b-4 border-black transition-all">返回主介面</button>
                </div>
              </div>
            </div>
          )}
        ...
        {showSettings && (
          <div className="absolute inset-0 bg-black/98 z-[300] flex flex-col p-8 animate-in fade-in backdrop-blur-3xl overflow-y-auto">
            <div className="w-full max-w-xl mx-auto space-y-10 pt-4 pb-12">
              <div className="flex justify-between items-center border-b border-zinc-800 pb-6">
                <h2 className="text-2xl sm:text-3xl font-header text-white uppercase tracking-widest flex items-center gap-4"><Settings size={28} className="text-cyan-400 sm:w-[32px]" /> 系統設置</h2>
                <button onClick={() => setShowSettings(false)} className="p-2 bg-zinc-800 rounded-xl text-zinc-400"><X /></button>
              </div>
              <div className="space-y-8 bg-zinc-900/50 p-6 sm:p-8 rounded-[40px] border border-zinc-800">
        ...
        {showHistory && (
          <div className="absolute inset-0 bg-black/98 z-[300] flex flex-col animate-in slide-in-from-bottom">
            <div className="p-4 border-b-2 border-zinc-800 flex justify-between items-center bg-zinc-900 shrink-0">
              <h2 className="text-xl font-header text-cyan-400 flex items-center gap-3 uppercase"><History /> 任務日誌</h2>
